...
export const buildFilter = (query) => {
  const {
    limit = 20, offset = 0, order = null, tagsQuery = '', statusId = 'all', assigneeId = 'all', creatorId = 'all',
  } = query;
  const tags = getTagsFromQuery(tagsQuery);
  const filterProperties = [
    {
      key: 'limit',
      value: () => (limit === 20 ? limit : Number(limit)),
    },
    {
      key: 'offset',
...
export const getFilteredTasks = filter => (
  (filter.tags === null || filter.tags.length <= 1)
    ? Task.scope({ method: ['filtered', filter] }).findAll()
    : Task.scope({ method: ['filtered', filter] }, { method: ['tags', filter.tags] }).findAll()
);
