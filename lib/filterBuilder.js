...
export default (query) => {
  ...
  const priorFilter = { group: '', having: '', subQuery: false, offset, limit: 10, order: [[orderBy, orderDirection]] };
  ...
  if (tags.length > 1) {
    ...
    // work-around for PostgreSQL, more on that => https://github.com/sequelize/sequelize/issues/3256
    // const newInclude = filter.include.map(obj => ({ ...obj, attributes: [] }));
    // filter.include = newInclude;
    // filter.raw = true;
  }
  console.log(filter);
  return filter;
