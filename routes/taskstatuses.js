...
export default (router) => {
  router
    ...
    .get('statusEdit', '/taskStatuses/:id/edit', userAuth, async (ctx) => {
      const { id } = ctx.params;
      const status = await TaskStatus.findByPk(id);
      ctx.render('taskStatuses/edit', { f: buildFormObj(status) });
    })
    ...
    .patch('modifyStatus', '/taskStatuses/:id', userAuth, async (ctx) => {
      ...
      try {
        ...
        ctx.render('taskStatuses/edit', { f: buildFormObj(status, err) });
      }
    })
